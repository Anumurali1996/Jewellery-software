...
import admindash
from emplogin import emplogin_window
...
class login_window:
    def __init__(self,root):
        ...
        emp_btn=Button(frame,text="Login as Employee",command=self.emp_login,font=("times new roman",12,"bold"),borderwidth=0,fg="black",bg="goldenrod",cursor="hand2",activebackground="goldenrod")
        emp_btn.place(x=50,y=350)
    ...
    def emp_login(self):
        ...
        self.emplog=emplogin_window(self.new_window)
    ...
    def login_win(self):
        ...
        if self.txtusername.get()=="" or self.txtpassword.get()=="":
            ...
            conn=mysql.connector.connect(host='localhost',username='root',password='Anu@17151431',database='jewellery')
            my_cursor=conn.cursor()
            my_cursor.execute("select * from adminregister where user_name=%s and pass_word=%s",(
            ...
            if row==None:
                ...
                if open_main>0:
                    ...
                    self.dash=admindash.dashboard(self.new_window)
    ...
    def reset_password(self):
        if self.combo_security_q.get()=="Select":
            ...
            conn=mysql.connector.connect(host='localhost',username='root',password='Anu@17151431',database='jewellery')
            my_cursor=conn.cursor()
            qury=("select * from adminregister where user_name=%s and securityQ=%s and securityA=%s")
            ...
            if row==None:
                ...
                query=("update adminregister set pass_word=%s where user_name=%s")
    ...
    def for_pass_win(self):
        if self.txtusername.get()=="":
            ...
            conn=mysql.connector.connect(host='localhost',username='root',password='Anu@17151431',database='jewellery')
            my_cursor=conn.cursor()
            query=("select * from adminregister where user_name=%s")
